...
test_docker_availability() {
    echo "Testing Docker availability for cross-compilation..."
    ...
    if command -v docker >/dev/null 2>&1; then
        ...
        if docker info >/dev/null 2>&1; then
            ...
            if docker ps >/dev/null 2>&1; then
                echo "Docker permissions: OK (rootless or in docker group)"
            else
                echo "Docker permissions: WARNING (may require sudo or adding user to docker group)"
...
# Test Docker availability and cross-compilation options
test_docker_availability
