...
namespace GGUI{
    ...
    namespace INTERNAL{
        ...
        HTMLNode* elementToNode(element* e){
            ...
            Result->Attributes["width"] = new GGUI::INTERNAL::HTMLToken(HTML_GROUP_TYPES::NUMBER, std::to_string(e->getWidth()));
            Result->Attributes["height"] = new GGUI::INTERNAL::HTMLToken(HTML_GROUP_TYPES::NUMBER, std::to_string(e->getHeight()));
        ...
        void translateAttributesToElement(element* e, HTMLNode* input){
            ...
            [[maybe_unused]] GGUI::INTERNAL::HTML_POSITION_TYPE Current_Position_Type = GGUI::INTERNAL::HTML_POSITION_TYPE::STATIC;
            ...
            for (auto attr : input->Attributes){
                ...
                if (attr.first == "width")
                    e->setWidth(GGUI::INTERNAL::computeVal(attr.second, input->parent, attr.first));
                // Set the height of the Element to the value of the height attribute
                else if (attr.first == "height")
                    e->setHeight(GGUI::INTERNAL::computeVal(attr.second, input->parent, attr.first));
                // Check if the attribute is a flex property
                    ...
                    if (!USE_FLEX){
                        ...
                        GGUI::report("Cannot use Flex properties whitout enabling flexbox first!");
                        continue;
                    }
                    ...
                    if (attr.first == "flex-direction"){
                        if (attr.second->Data == "column")
                            ...
                            GGUI::report("Unknown flex-direction: " + attr.second->Data);
                    }
                }
            }
        }
        ...
        void translateChildsToElement(element* e, HTMLNode* input, std::string* Set_Text_To) {
            ...
            for (auto c : input->Childs) {
                ...
                if (GGUI::INTERNAL::HTMLTranslators->find(c->Tag_Name) == GGUI::INTERNAL::HTMLTranslators->end())
                    continue;

                // Translate the child node to an Element and add it to the parent Element.
                GGUI::element* tmp = GGUI::INTERNAL::HTMLTranslators->at(c->Tag_Name)(c);
                if (tmp) {
                    e->addChild(tmp);
                }
            }
