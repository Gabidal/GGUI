        ...
        namespace tcp {
            ...
            public:
                ...
                explicit connection(int socketFd) : handle(socketFd) {
                    if (socketFd < 0) {
                        GGUI::logger::log("Invalid socket file descriptor provided to connection constructor");
                    }
                }
                ...
                bool Send(const T* data, size_t count = 1) {
                    if (handle < 0) {
                        GGUI::logger::log("Cannot send on closed socket");
                    }
                    if (!data && count > 0) {
                        return false;
                    }
                ...
                bool receive(T* data, size_t count = 1) {
                    if (handle < 0) {
                        GGUI::logger::log("Cannot receive on closed socket");
                    }
                    if (!data && count > 0) {
                        return false;
                    }
            ...
            public:
                ...
                explicit listener(uint16_t port) : handle(-1) {
                    ...
                    if (handle < 0) {
                        GGUI::logger::log("Failed to create socket: " + std::string(strerror(errno)));
                    }
                    ...
                    if (setsockopt(handle, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) < 0) {
                        ::close(handle);
                        GGUI::logger::log("Failed to set socket option SO_REUSEADDR: " + std::string(strerror(errno)));
                    }
                    ...
                    if (bind(handle, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) < 0) {
                        ::close(handle);
                        GGUI::logger::log("Failed to bind socket to port " + std::to_string(port) + ": " + std::string(strerror(errno)));
                    }

                    // Start listening
                    if (listen(handle, 5) < 0) {
                        ::close(handle);
                        GGUI::logger::log("Failed to start listening on socket: " + std::string(strerror(errno)));
                    }
                }
                ...
                connection Accept() {
                    if (handle < 0) {
                        GGUI::logger::log("Cannot accept on closed listener");
                    }

                    int connFd = accept(handle, nullptr, nullptr);
                    if (connFd < 0) {
                        GGUI::logger::log("Failed to accept connection: " + std::string(strerror(errno)));
                    }
                ...
                uint16_t getPort() {
                    if (handle < 0) {
                        GGUI::logger::log("Cannot get port of closed listener");
                    }
                    ...
                    if (getsockname(handle, reinterpret_cast<sockaddr*>(&actual), &len) < 0) {
                        GGUI::logger::log("Failed to get socket name: " + std::string(strerror(errno)));
                    }
            ...
            public:
                ...
                static connection getConnection(uint16_t port, const char* host = "127.0.0.1") {
                    ...
                    if (result <= 0) {
                        ::close(sockFd);
                        if (result == 0) {
                            GGUI::logger::log("Invalid IP address format: " + std::string(host));
                        } else {
                            GGUI::logger::log("inet_pton failed: " + std::string(strerror(errno)));
                        }
                    }
                    ...
                    if (connect(sockFd, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) < 0) {
                        ::close(sockFd);
                        GGUI::logger::log("Failed to connect to " + std::string(host) + ":" + std::to_string(port) + " - " + std::string(strerror(errno)));
